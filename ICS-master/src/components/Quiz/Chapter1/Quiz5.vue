...
export default {
    ...
    methods:{ 
        ...
        GetQA:function(){     
          var self = this
          let QuestionRef = db.collection('Q&A').doc('Question&Answer').collection('Quiz').doc('Chapter1');
          ...
          if(this.choice!=null){
            ...
            if(parseInt(this.choice)==self.ArrayQ.Number5.Anum){
             ...
             const item={
                ...
                db.collection('UserData').doc('Quiz').collection('QuizChapter1').doc(self.userTable).update(item).then(res=>{
                      self.QuizNumber5="";
                 })
                //  console.log(self.choice,self.ArrayQ.Anum)
                 self.$router.push({name: 'Chapter1QuizScore'})
                 } 
                },
