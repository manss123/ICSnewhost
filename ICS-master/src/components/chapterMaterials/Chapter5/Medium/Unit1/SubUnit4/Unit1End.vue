        ...
        <div class="container col s12 m12 l12" style="margin-top:1%;">    
            ...
            <div class="col s12 m12 l12 row">
                    ...
                    <div class="right col s12 m12 l6 hide-on-small-only">
                                <router-link :to="{name:'Chapter5'}"><a class="waves-effect red darken-1 waves-light flow-text btn-large align-center right"> ต่อไป </a></router-link>
                    </div>
                    <div class="right col s12 m12 l6 hide-on-med-and-up">
                                <router-link :to="{name:'Chapter5'}"><a class="waves-effect red darken-1 waves-light flow-text btn-small align-center right"> ต่อไป </a></router-link>
                    </div> 
    ...
    methods:{
        ...
        GetEmail:function(userTable){
            ...
            StatusRef.get(getOptions).then(function(doc){
                ...
                let SequenceRef = db.collection('UserData').doc('Sequence').collection('Chapter5Count').doc(userTable)
         ...
         UpdateStatus:function(userTable,RecordTime,SequenceData,UnitTime){
            let self = this;
            var getOptions={ source: 'server'};
            let LoggingRef = db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit1').collection('Level').doc('Medium').collection('Chapter5Unit1MediumLoging').doc(userTable).collection('Chapter5Unit1MediumSequence').doc(UnitTime)
            ...
            LoggingRef.get(getOptions).then(function(doc){
                ...
                if(Total<25){
                    ...
                    db.collection('UserData').doc('Status').collection('Chapter5').doc(userTable).update(NextStatus)
                    db.collection('UserData').doc('Record').collection('Chapter5').doc(userTable).collection('Time').doc(RecordTimeString).set(RecordTime)
               ...
               db.collection('UserData').doc('Sequence').collection('Chapter5Count').doc(userTable).update({Time: self.UnitTime+1})
                ...
                const ProgressUpdate={
                    ...
                    db.collection('UserData').doc('Progress').collection('Chapter5').doc(self.userTable).update(ProgressUpdate)
            })
        },
