        ...
        <div class="row card-panel red accent-1 s12 m12 l12 valign-wrapper black-text" style="margin-top:10px;">
         ...
         <div class="row container col s12 m6 l6 black-text valign-wrapper" style="margin-bottom:2%;">
                    <div class="box sb1 col s6 m6 l5 flow-text left-align" style="padding:2%;margin-bottom:1%;font-weight:bold;">
                            ถ้าพี่โมโม่นั่งเล่น เฟสบุ๊คอยู่ หลังจากนั้นพี่โมโม่ไม่ สามารถเปิดไฟล์ได้เลย แต่มีข้อความแจ้งเตือนว่า คอมพิวเตอร์ของคุณมีปัญหา ถ้าต้องการแก้ไข พี่โมโม่ต้อง จ่ายเงิน น้องๆว่าเกิดจากอะไร ครับ ช่วยพี่โมโม่หน่อยครับ
                    </div>
        ...
        <div class="container col s12 m12 l12">
            <div class="card-panel col s12 m12 l12 red accent-1 white-text row" style="margin-top:4%;">
                        <div class="card-panel red darken-1 lighten-3 col s12 m12 l12 center-align">
                                ...
                                <p class="flow-text">เกิดจาก วินโดว หมดอายุ</p>
                        ...
                        <div class="card-panel red darken-1 lighten-3 col s12 m12 l12 center-align">
                                ...
                                <p class="flow-text">เกิดจากไวรัส</p>
                                <div class="center col s12 m12 l12" style="margin-bottom:10px;">
                        ...
                        <div class="card-panel red darken-1 lighten-3 col s12 m12 l12 center-align">
                                ...
                                <p class="flow-text">เกิดจากแรนซัมแวร์</p>
                                <div class="center col s12 m12 l12" style="margin-bottom:10px;">
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit1').collection('Level').doc('Hard').collection('Chapter5Unit1HardLoging').doc(Email).collection('Chapter5Unit1HardSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit1').collection('Level').doc('Hard').collection('Chapter5Unit1HardLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        .limit(25);
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C5Unit1HardRound2P4'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
