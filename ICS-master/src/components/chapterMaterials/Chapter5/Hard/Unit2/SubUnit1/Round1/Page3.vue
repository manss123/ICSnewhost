            ...
            <div class="card-panel row red darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    ...
                    <div class="right col s12 m12 l6 flow-text">
                        <router-link :to="{name:'C5Unit2HardRound1P4'}"><a @click="submit" class="waves-effect grey darken-1 waves-light btn-large align-center right"> ส่งคำตอบ </a></router-link>
                    </div>
                    </div>
                </div>
            </div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C5Unit2HardRound1P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C5Unit2HardRound1P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit2').collection('Level').doc('Hard').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit1').collection('Level').doc('Hard').collection('Chapter5Unit2HardLoging').doc(Email).collection('Chapter5Unit2HardSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })
            db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit2').collection('Level').doc('Hard').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit2').collection('Level').doc('Hard').collection('Chapter5Unit2HardLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
