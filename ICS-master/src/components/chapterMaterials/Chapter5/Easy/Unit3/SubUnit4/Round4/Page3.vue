<template>
    <div class="unit black-text">
        <div class="container center col s12 m12 l12" style="margin-top:2%">
            <ul class="pagination">
            ...
            <div class="card-panel row red darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;"> น้องๆเคยใช้งาน VPN หรือไม่ครับ </p></div>
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    ...
                    <div class="left col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s12 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> เคย </a>
                    </div>
                    ...
                    <div class="right col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s12 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่เคย </a><br>
                    </div>
                </div>
            </div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C5Unit3EasyRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C5Unit3EasyRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit3').collection('Level').doc('Easy').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit1').collection('Level').doc('Easy').collection('Chapter5Unit3EasyLoging').doc(Email).collection('Chapter5Unit3EasySequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter5').doc('Unit3').collection('Level').doc('Easy').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter5').doc('Unit3').collection('Level').doc('Easy').collection('Chapter5Unit3EasyLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C5Unit3EasyRound4P4'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
