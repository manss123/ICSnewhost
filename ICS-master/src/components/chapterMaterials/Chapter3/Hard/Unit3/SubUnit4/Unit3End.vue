    ...
    methods:{
        ...
        GetEmail:function(userTable){
         ...
         UpdateStatus:function(userTable,RecordTime,SequenceData,UnitTime){
            let self = this;
            var getOptions={ source: 'server'};
            let LoggingRef = db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit1').collection('Level').doc('Hard').collection('Chapter3Unit3HardLoging').doc(userTable).collection('Chapter3Unit3HardSequence').doc(UnitTime)
            
            LoggingRef.get(getOptions).then(function(doc){
                console.log(doc.data().Round1Log.Correct)
