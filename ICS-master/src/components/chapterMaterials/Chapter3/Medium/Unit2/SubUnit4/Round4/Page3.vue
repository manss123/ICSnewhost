    ...
    <div class="unit black-text">
        <div class="container col s12 m12 l12 center" style="margin-top:2%">
            ...
            <div class="card-panel row  light-blue darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;"> น้องๆเคยกลั่นแกล้งเพื่อนๆของน้องทางสังคมออนไลน์หรือป่าวครับ </p></div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C3Unit2MediumRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C3Unit2MediumRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter3').doc('Unit2').collection('Level').doc('Medium').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit1').collection('Level').doc('Medium').collection('Chapter3Unit2MediumLoging').doc(Email).collection('Chapter3Unit2MediumSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter3').doc('Unit2').collection('Level').doc('Medium').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit2').collection('Level').doc('Medium').collection('Chapter3Unit2MediumLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C3Unit2MediumRound4P4'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
