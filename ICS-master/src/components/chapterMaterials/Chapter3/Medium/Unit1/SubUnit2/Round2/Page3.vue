    ...
    <div class="unit black-text">
        <div class="container col s12 m12 l12 center" style="margin-top:2%">
            ...
            <div class="card-panel row light-blue darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;"> ถ้าพี่โมโม่ถูกไล่ออกจากกลุ่มไลน์ เพราะว่าคนในกลุ่มพยายามจะกีดกันพี่โมโม่ แสดงว่าพี่โมโม่ถูกกลั่นแกล้งทางสังคมออนไลน์ (Cyberbullying) หรือไม่ครับ </p></div>
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    <div class="left col s6 m12 l6 hide-on-small-only">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> ใช่ </a>
                    </div>
                    <div class="left col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s10 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> ใช่</a>
                    </div>
                    <div class="right col s6 m12 l6 hide-on-small-only">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่ใช่</a><br>
                    </div>
                    <div class="right col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s10 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่ใช่ </a><br>
                    </div>
                </div>
            </div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C3Unit1MediumRound2P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C3Unit1MediumRound2P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter3').doc('Unit1').collection('Level').doc('Medium').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit1').collection('Level').doc('Medium').collection('Chapter3Unit1MediumLoging').doc(Email).collection('Chapter3Unit1MediumSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter3').doc('Unit1').collection('Level').doc('Medium').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit1').collection('Level').doc('Medium').collection('Chapter3Unit1MediumLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C3Unit1MediumRound2P4'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
