<template>
        <div class="container col s12 m12 l12 row">
                ...
                <div class="row col s12 m12 l12" style="margin-top:30px">
                    ...
                    <div class="col s6 m6 l6 hide-on-med-and-up">
                            <router-link :to="{name:'C3Unit2EasyRound1P1'}"><a @click="submit" class="waves-effect light-blue waves-light btn-small  right align-center"> เข้าสู่บทเรียน </a></router-link>
                    </div>
                    <div class="col s6 m6 l6 hide-on-small-only">
                            <router-link :to="{name:'C3Unit2EasyRound1P1'}"><a @click="submit" class="waves-effect light-blue waves-light btn-large right align-center"> เข้าสู่บทเรียน </a></router-link>
                    </div>
                 </div>
        </div>
</template>
    ...
    methods:{
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            db.collection('UserData').doc('Content').collection('Chapter3').doc('Unit2').collection('Level').doc('Easy').collection('Content').doc(self.userTable).set(item).then(res=>{
        ...
        Start:function(email){
            ...
            db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit2').collection('Level').doc('Easy').collection('Chapter3Unit2EasyLoging').doc(this.email).set(item).then(res=>{
                    self.Starting=""; 
            })
        },
