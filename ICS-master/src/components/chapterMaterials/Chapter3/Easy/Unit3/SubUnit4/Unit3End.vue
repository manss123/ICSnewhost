    ...
    methods:{
        ...
        GetEmail:function(userTable){
         ...
         UpdateStatus:function(userTable,RecordTime,SequenceData,UnitTime){
            let self = this;
            var getOptions={ source: 'server'};
            let LoggingRef = db.collection('UserData').doc('Loging').collection('Chapter3').doc('Unit1').collection('Level').doc('Easy').collection('Chapter3Unit3EasyLoging').doc(userTable).collection('Chapter3Unit3EasySequence').doc(UnitTime)
