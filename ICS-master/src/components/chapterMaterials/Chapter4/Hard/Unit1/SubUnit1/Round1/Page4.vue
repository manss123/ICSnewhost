    ...
    <div class="unit container black-text">
        <div class="container col s12 m12 l12 center" style="margin-top:2%">    
            ...
            <div class="card-panel row light-green darken-1 col s12 m12 l12 white-text" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div @click="FollowLink()"><router-link :to="{name:'C4Unit1HardRound1P1'} "><a @click="submit" class="waves-effect grey darken-1 waves-light flow-text btn-large align-center col s6 m4 l4 offset-s3 offset-m4 offset-l4" style="margin-top:10px;"> ทบทวนความรู้ </a></router-link></div>
            ...
            <div class="right row col s6 m6 l6">
             ...
             <div class="col s12 m12 l12 feedback" style="margin-top:80px;margin-bottom:230px;">
                ...
                <div class="right col s6 m12 l6 hide-on-small-only">
                    <router-link :to="{name:'C4Unit1HardRound2P1'}"><a @click="submit" class="waves-effect light-green darken-1 waves-light flow-text btn-large align-center right"> ต่อไป </a></router-link>
                </div>
                <div class="right col s6 m12 l6 hide-on-med-and-up">
                    <router-link :to="{name:'C4Unit1HardRound2P1'}"><a @click="submit" class="waves-effect light-green darken-1 waves-light flow-text btn-small align-center right"> ต่อไป </a></router-link>
                </div> 
    ...
    methods:{
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            db.collection('UserData').doc('Loging').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Chapter4Unit1HardLoging').doc(self.email).update(item2).then(res=>{
                    self.Round1Log=""; 
            })
              });
                 ...
                 var getOptions = {
                    ...
                    .then(function(querySnapshot) {
                        querySnapshot.forEach(function(doc) {
                            // doc.data() is never undefined for query doc snapshots
                            self.userTable = doc.id
                            let Qref = db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(self.userTable);
        ...
        FollowLinkCheck:function(email){
            ...
            let LogRef = db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(email)
            ...
            db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(email).update(item).then(res=>{
                    self.Round1Page4Log=""; 
            })
            db.collection('UserData').doc('Loging').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Chapter4Unit1HardLoging').doc(email).collection('Chapter4Unit1HardSequence').doc(UnitTime).update(item2).then(res=>{
        ...
        FollowLink:function(){
           ...
           const item={
            ...
            db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Content').doc(self.email).update(item).then(res=>{
                    self.Round1Page4Log=""; 
            })
            db.collection('UserData').doc('Loging').collection('Chapter4').doc('Unit1').collection('Level').doc('Hard').collection('Chapter4Unit1HardLoging').doc(self.email).collection('Chapter4Unit1HardSequence').doc(UnitTime).update(item2).then(res=>{
                    self.Round1FollowLink=""; 
            })
                })
        },
