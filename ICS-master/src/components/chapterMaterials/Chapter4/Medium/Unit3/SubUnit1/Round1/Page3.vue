<template>
    <div class="unit black-text">
        <div class="container col s12 m12 l12" style="margin-top:2%">
            ...
            <div class="card-panel row  light-green darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;"> เอ พี่โมโม่อยากรู้จังเลย ถ้ามีข้อความส่งมาหาน้องว่า ได้รับเงิน 100,000 บาท กรุณาลงทะเบียนเพื่อรับสิทธิ น้องกดลงทะเบียนหรือไม่ </p></div>
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    <div class="left col s6 m12 l6 hide-on-small-only">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> กดลงทะเบียนเลย </a>
                    </div>
                    <div class="left col s12 m12 l6 hide-on-med-and-up">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s12 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> กดลงทะเบียนเลย </a>
                    </div>
                </div>
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    <div class="right col s6 m12 l6 hide-on-small-only">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่กดลงทะเบียน </a><br>
                    </div>
                    <div class="right col s12 m12 l6 hide-on-med-and-up">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s12 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่กดลงทะเบียน </a><br>
                    </div>
                </div>
            </div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C4Unit3MediumRound1P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C4Unit3MediumRound1P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit3').collection('Level').doc('Medium').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter4').doc('Unit1').collection('Level').doc('Medium').collection('Chapter4Unit3MediumLoging').doc(Email).collection('Chapter4Unit3MediumSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })
            db.collection('UserData').doc('Content').collection('Chapter4').doc('Unit3').collection('Level').doc('Medium').collection('Content').doc(Email).update(item)
            db.collection('UserData').doc('Loging').collection('Chapter4').doc('Unit3').collection('Level').doc('Medium').collection('Chapter4Unit3MediumLoging').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C4Unit3MediumRound1P4'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
