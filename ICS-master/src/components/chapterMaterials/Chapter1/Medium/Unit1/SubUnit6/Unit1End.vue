        ...
        <div class="container col s12 m12 l12" style="margin-top:1%;">    
            ...
            <div class="col s12 m12 l12 row">
                    ...
                    <div class="right col s12 m12 l6 hide-on-small-only">
                                <router-link :to="{name:'Chapter1'}"><a class="waves-effect teal darken-1 waves-light flow-text btn-large align-center right"> ต่อไป </a></router-link>
                    </div>
                    <div class="right col s12 m12 l6 hide-on-med-and-up">
                                <router-link :to="{name:'Chapter1'}"><a class="waves-effect teal darken-1 waves-light flow-text btn-small align-center right"> ต่อไป </a></router-link>
                    </div> 
    ...
    methods:{
        ...
        GetEmail:function(userTable){
         ...
         UpdateStatus:function(userTable,RecordTime,SequenceData,UnitTime){
            let self = this;
            var getOptions={ source: 'server'};
            let LoggingRef = db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Medium').collection('Chapter1Unit1MediumLoging').doc(userTable).collection('Chapter1Unit1MediumSequence').doc(UnitTime)
            ...
            LoggingRef.get(getOptions).then(function(doc){
                ...
                let Unit1Time = doc.data().Round1Page1Time.AllSecond + doc.data().Round1Page2Time.AllSecond + doc.data().Round2Page1Time.AllSecond + doc.data().Round2Page2Time.AllSecond
                            + doc.data().Round3Page1Time.AllSecond + doc.data().Round3Page2Time.AllSecond + doc.data().Round3Page3Time.AllSecond + doc.data().Round4Page1Time.AllSecond +
                             doc.data().Round4Page2Time.AllSecond + doc.data().Round4Page3Time.AllSecond + doc.data().Round5Page1Time.AllSecond + doc.data().Round5Page2Time.AllSecond + 
                             doc.data().Round6Page1Time.AllSecond + doc.data().Round6Page2Time.AllSecond;
                let Unit1Score = doc.data().Round2Log.Correct+ doc.data().Round3Log.Correct + doc.data().Round5Log.Correct;
                ...
                if(Total<25){
                    ...
                    db.collection('UserData').doc('Status').collection('Chapter1').doc(userTable).update(NextStatus)
                    db.collection('UserData').doc('Record').collection('Chapter1').doc(userTable).collection('Time').doc(RecordTimeString).set(RecordTime)
               ...
               db.collection('UserData').doc('Sequence').collection('Chapter1Count').doc(userTable).update({Time: self.UnitTime+1})
                ...
                const ProgressUpdate={
                    ...
                    db.collection('UserData').doc('Progress').collection('Chapter1').doc(self.userTable).update(ProgressUpdate)
            })
        },
