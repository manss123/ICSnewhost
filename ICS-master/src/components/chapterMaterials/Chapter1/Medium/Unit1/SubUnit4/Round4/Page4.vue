    ...
    <div class="unit container black-text col s12 m12 l12 center">
        <div class="container col s12 m12 l12" style="margin-top:2%">
            ...
            <div class="card-panel row teal darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;"> เป็นอย่างไรบ้างครับน้องๆ หลังจากรู้วิธีการป้องกันคอมพิวเตอร์จากไวรัส มัลแวร์หรือภัยคุกคามอื่นๆจากพี่โมโม่แล้ว น้องๆคิดว่าตนเองได้ลองอัพเดตคอมพิวเตอร์ของตนเองหรือยังครับ? </p></div>
                <div class="col s12 m12 l12 row valign-wrapper" style="margin:0px;margin-top:20px;">
                    <div class="left col s6 m12 l6 hide-on-small-only">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> เคยอัพเดต </a>
                    </div>
                    <div class="left col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit1" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s10 m5 l5 left center-align" style="margin-left:0px;font-weight:bold;"> เคยอัพเดต</a>
                    </div>
                    <div class="right col s6 m12 l6 hide-on-small-only">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-large col s6 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่เคยอัพเดต </a><br>
                    </div>
                    <div class="right col s6 m12 l6 hide-on-med-and-up">
                        <a @click="submit2" class="waves-effect waves-light flow-text white-text  grey darken-1 btn-small col s10 m5 l5 right center-align" style="margin-right:0px;font-weight:bold;"> ไม่เคยอัพเดต </a><br>
                    </div>
                </div>
            </div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C1Unit1MediumRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C1Unit1MediumRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Medium').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            let StartHourP3 = doc.data().Round4Page3Start.StartTimeHour;
            let EndHourP3 = doc.data().Round4Page3End.EndTimeHour;
            let FinalHourP3 = Math.abs(EndHourP3-StartHourP3)*3600;

            let StartMinuteP3 = doc.data().Round4Page3Start.StartTimeMinute;
            let EndMinuteP3 = doc.data().Round4Page3End.EndTimeMinute;
            let FinalMinuteP3 = Math.abs(EndMinuteP3-StartMinuteP3)*60;

            let StartSecondP3 = doc.data().Round4Page3Start.StartTimeSecond;
            let EndSecondP3 = doc.data().Round4Page3End.EndTimeSecond;
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Medium').collection('Chapter1Unit1MediumLoging').doc(Email).collection('Chapter1Unit1MediumSequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Medium').collection('Content').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Medium').collection('Content').doc(self.userTable).update(item).then(res=>{
                      self.Round4Page4End="";
                      self.Round4Page5Start="";
                      
            })
            self.$router.push({name: 'C1Unit1MediumRound4P5'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
