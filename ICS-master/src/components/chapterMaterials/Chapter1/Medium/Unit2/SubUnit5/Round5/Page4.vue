    ...
    <div class="unit container black-text center col s12 m12 l12">
        ...
        <div class="container col s12 m12 l12" style="margin-top:2%">    
            ...
            <div class="card-panel row teal darken-1 col s12 m12 l12 white-text" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                <div v-if="usedTo==false" class="flow-text left col s12 m12 l12" style="font-weight:bold;">แม้น้องจะไม่เคยเจอ แต่ยังมีการเข้าถึงข้อมูลของน้องในหลายแบบเช่นกัน ตามพี่โมโม่มาเลยครับ</div>
                <div v-if="usedTo==true" class="flow-text left col s12 m12 l12" style="font-weight:bold;">งั้นเรามาดูกันใหม่ว่า สิ่งที่น้องๆสังเกตเห็นนั้นจะตรงกับที่พี่โมโม่สังเกตเห็นด้วยหรือไม่</div>
                <div><router-link :to="{name:'C1Unit2MediumRound5P1'} "><a @click="submit" class="waves-effect grey darken-1 waves-light flow-text btn-large align-center col s6 m4 l4 offset-s3 offset-m4 offset-l4" style="margin-top:10px;"> ทบทวนความรู้ </a></router-link></div>
            ...
            <div class="right row col s6 m6 l6">
             ...
             <div class="col s12 m12 l12 feedback" style="margin-top:80px;margin-bottom:230px;">
                ...
                <router-link :to="{name:'Unit2MedRound5P1'}"><a v-if="usedTo==false" class="waves-effect teal darken-1 waves-light flow-text btn-large align-center left"> ย้อนกลับ </a></router-link>
                </div>
                <div class="left col s6 m12 l6 hide-on-med-and-up">
                    <router-link :to="{name:'Unit2MedRound5P1'}"><a v-if="usedTo==false" class="waves-effect teal darken-1 waves-light flow-text btn-small align-center left"> ย้อนกลับ </a></router-link>
                </div>

                <div class="left col s6 m12 l6 hide-on-small-only">
                <router-link :to="{name:'Unit2MedRound5P1'}"><a v-if="usedTo==true" class="waves-effect teal darken-1 waves-light flow-text btn-large align-center left"> ทบทวนความรู้ </a></router-link>
                </div>
                <div class="left col s6 m12 l6 hide-on-med-and-up">
                    <router-link :to="{name:'Unit2MedRound5P1'}"><a v-if="usedTo==true" class="waves-effect teal darken-1 waves-light flow-text btn-small align-center left"> ทบทวนความรู้ </a></router-link>
                </div> -->

                <div class="right col s6 m12 l6 hide-on-small-only">
                    <router-link :to="{name:'C1Unit2MediumEnd'}"><a @click="submit" class="waves-effect teal darken-1 waves-light flow-text btn-large align-center right"> ต่อไป </a></router-link>
                </div>
                <div class="right col s6 m12 l6 hide-on-med-and-up">
                    <router-link :to="{name:'C1Unit2MediumEnd'}"><a @click="submit" class="waves-effect teal darken-1 waves-light flow-text btn-small align-center right"> ต่อไป </a></router-link>
                </div> 
                 ...
                 var getOptions = {
                    ...
                    .then(function(querySnapshot) {
                        ...
                        Qref.get(getOptions).then(function(doc){
                            if(doc.data().Round5Page3End.Choice==1){
                                self.usedTo = true;
                            }else{
                                self.usedTo = false;
                            }
