        ...
        <div class="container col s12 m12 l12" style="margin-top:2%">
            ...
            <li class="waves-effect"><a href="#!">6</a></li>
             ...
             <div class="row card-panel teal accent-2 s12 m12 l12 valign-wrapper black-text" style="margin-top:10px;">
                    ...
                    <div class="left-align flow-text col s12 m8 l8 left"><h5 style="font-weight:bold;"> สะกิดความคิด<br>ชีวิตพลิกแพลง </h5></div> 
            </div>
            ...
            <div class="card-panel row teal darken-1 col s12 m12 l12" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div class="col s12 m12 l12"> <p class="flow-text center white-text" style="font-weight:bold;font-size:16px;"> เป็นอย่างไรบ้างครับน้องๆ หลังจากรู้วิธีการป้องกันคอมพิวเตอร์จากไวรัส มัลแวร์หรือภัยคุกคามอื่นๆจากพี่โมโม่แล้ว น้องๆคิดว่าตนเองได้ลองอัพเดตคอมพิวเตอร์ของตนเองหรือยังครับ? </p></div>
                ...
                <!-- <div class="left col s12 m12 l12">
                        <router-link :to="{name:'C1Unit1EasyRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-large hide-on-small-only align-center left"> ย้อนกลับ </a></router-link>
                        <router-link :to="{name:'C1Unit1EasyRound4P2'}"><a @click="submit" style="" class="waves-effect cyan darken-4 waves-light flow-text btn-small hide-on-med-and-up align-center left"> ย้อนกลับ </a></router-link>
                </div> -->
...
export default {
    ...
    methods:{
        ...
        TimeCal:function(Email){
            ...
            let ref = db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(Email);
        ref.get(getOptions).then(function(doc){
            ...
            SequenceRef.get().then(function(doc){
             ...
             db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Chapter1Unit1EasyLoging').doc(Email).collection('Chapter1Unit1EasySequence').doc(self.UnitTime).update(item).then(res=>{
                    self.UnitTime=""; 
            })
            })

            db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(Email).update(item)
            // // console.log(StartSecond,EndSecond);
 ...
 // ********* check user id
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            self.$router.push({name: 'C1Unit1EasyRound4P5'})
              });
          })
          .catch(function(error) {
              console.log("Error getting documents: ", error);
