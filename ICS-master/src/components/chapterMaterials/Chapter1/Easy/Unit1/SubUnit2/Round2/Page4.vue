    ...
    <div class="unit container black-text center col s12 m12 l12">
        ...
        <div class="container col s12 m12 l12" style="margin-top:2%">    
            ...
            <div class="card-panel row teal darken-1 col s12 m12 l12 white-text" style="padding:10px;padding-bottom:10px;margin-top:30px;">
                ...
                <div @click="FollowLink()"><router-link :to="{name:'C1Unit1EasyRound2P1'} "><a @click="submit" class="waves-effect grey darken-1 waves-light flow-text btn-large align-center col s6 m4 l4 offset-s3 offset-m4 offset-l4" style="margin-top:10px;"> ทบทวนความรู้ </a></router-link></div>
            ...
            <div class="right row col s6 m6 l6">
             ...
             <div class="col s12 m12 l12 feedback" style="margin-top:80px;margin-bottom:230px;">
                ...
                <div class="right col s6 m12 l6 hide-on-small-only">
                    <router-link :to="{name:'C1Unit1EasyRound3P1'}"><a @click="submit" class="waves-effect teal darken-1 waves-light flow-text btn-large align-center right"> ต่อไป </a></router-link>
                </div>
                <div class="right col s6 m12 l6 hide-on-med-and-up">
                    <router-link :to="{name:'C1Unit1EasyRound3P1'}"><a @click="submit" class="waves-effect teal darken-1 waves-light flow-text btn-small align-center right"> ต่อไป </a></router-link>
                </div> 
...
export default {
    ...
    methods:{
        ...
        FollowLinkCheck:function(email){
            ...
            let LogRef = db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(email)
            ...
            db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(email).update(item).then(res=>{
                    self.Round2Page4Log=""; 
            })
            db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Chapter1Unit1EasyLoging').doc(email).collection('Chapter1Unit1EasySequence').doc(UnitTime).update(item2).then(res=>{
        ...
        let Gusers = db.collection("Gusers").orderBy("Email")
          ...
          .then(function(querySnapshot) {
            ...
            db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Chapter1Unit1EasyLoging').doc(self.userTable).update(item2).then(res=>{
                    self.Round2Log=""; 
            })
              });
                 ...
                 var getOptions = {
                    ...
                    .then(function(querySnapshot) {
                        querySnapshot.forEach(function(doc) {
                            // doc.data() is never undefined for query doc snapshots
                            self.userTable = doc.id
                            let Qref = db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(self.userTable);
        ...
        FollowLink:function(){
           ...
           const item={
            ...
            db.collection('UserData').doc('Content').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Content').doc(self.email).update(item).then(res=>{
                    self.Round2Page4Log=""; 
            })
            db.collection('UserData').doc('Loging').collection('Chapter1').doc('Unit1').collection('Level').doc('Easy').collection('Chapter1Unit1EasyLoging').doc(self.email).collection('Chapter1Unit1EasySequence').doc(UnitTime).update(item2).then(res=>{
                    self.Round2FollowLink=""; 
            })
                })
        },
